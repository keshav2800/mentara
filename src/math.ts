...
export interface FeeSchedule {
  feeMode: number; // 0 flat | 1 linear | 2 exponential
  cliffFeeNum: number;
  periodMs: number;
  feeReduction: number;
  nPeriods: number;
...
export function currentFeeNum(
  s: FeeSchedule,
  activationMs: number,
  nowMs: number,
  isFirstSwap: boolean,
): number {
  if (isFirstSwap && s.firstSwapMinFee) return MIN_FEE_NUM;
  if (s.feeMode === 0) return s.cliffFeeNum;
  const elapsed = Math.max(0, nowMs - activationMs);
  const n = Math.min(Math.floor(elapsed / s.periodMs), s.nPeriods);
  if (s.feeMode === 1) return s.cliffFeeNum - s.feeReduction * n;
  let fee = s.cliffFeeNum;
  for (let i = 0; i < n; i++) fee = Math.floor((fee * (10_000 - s.feeReduction)) / 10_000);
  return fee;
}

/** ceil(amount · feeNum / denom) — fee rounding favours the pool. */
export function ceilFeeRaw(amountRaw: bigint, feeNum: number): bigint {
  return (amountRaw * BigInt(feeNum) + BigInt(FEE_DENOM - 1)) / BigInt(FEE_DENOM);
}

/** Spot price (quote per whole token) from a raw sqrt price. */
export function priceFromSqrt(sqrtRaw: bigint): number {
...
export function curveQuoteOut(segments: CurveSeg[], sqrtRaw: bigint, baseInRaw: number): number {
  let sigma = Number(sqrtRaw) / TWO64;
  let remaining = baseInRaw;
  let quoteOut = 0;
  for (let i = segments.length - 1; i >= 0 && remaining > 0; i--) {
    const s = segments[i]!;
    if (s.lower >= sigma) continue;
    const top = Math.min(s.upper, sigma);
    if (top <= s.lower) continue;
...
export function curveQuoteForBase(
  segments: CurveSeg[],
  sqrtRaw: bigint,
  baseTargetRaw: number,
): number {
  let sigma = Number(sqrtRaw) / TWO64;
...
export function ammBaseOut(baseReserve: bigint, quoteReserve: bigint, netQuoteRaw: bigint): bigint {
  return (baseReserve * netQuoteRaw) / (quoteReserve + netQuoteRaw);
}
export function ammQuoteOut(baseReserve: bigint, quoteReserve: bigint, tokenRaw: bigint): bigint {
...
export function solveSingleSegmentCurve(opts: {
  ...
  bufferPct: number; // contract's sell-back buffer (e.g. 25)
}): { sqrtStart: bigint; sqrtEnd: bigint; liquidity: bigint } {
