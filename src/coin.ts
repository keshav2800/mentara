...
export interface CoinTemplate {
  ...
  identifiers: { otw: string; module: string };
  /** The template's compiled-in default constant values (to match on patch). */
  defaults: { symbol: string; name: string; description: string; iconUrl: string };
}

export interface CoinMetadata {
  ticker: string; // becomes the module (lower) + OTW (upper) + symbol
  name: string;
  description: string;
  iconUrl: string;
}
...
export function patchCoinBytecode(template: CoinTemplate, meta: CoinMetadata): string {
  ...
  let bytecode: Uint8Array<ArrayBufferLike> = fromB64(template.bytecodeB64);
  ...
  const patch = (from: string, to: string) => {
    bytecode = update_constants(
      bytecode,
      bcs.vector(bcs.u8()).serialize(new TextEncoder().encode(to)).toBytes(),
      bcs.vector(bcs.u8()).serialize(new TextEncoder().encode(from)).toBytes(),
      'Vector(U8)',
    );
  };
 ...
 */
export function buildPublishCoinTx(patchedModuleB64: string, dependencies: string[]): Transaction {
...
export function coinTypeFor(packageId: string, ticker: string): string {
