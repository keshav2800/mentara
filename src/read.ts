...
import type { CurveSeg } from './math.js';
...
function readBalance(field: unknown): bigint {
  ...
  if (field && typeof field === 'object' && 'value' in field) {
    return BigInt(String((field as { value: string }).value));
  }
  return 0n;
}

async function fields(client: SuiClient, id: string): Promise<Record<string, unknown>> {
  const obj = await client.getObject({ id, options: { showContent: true } });
  const f = (obj.data?.content as { fields?: Record<string, unknown> })?.fields;
  if (!f) throw new Error(`object ${id} unreadable`);
  return f;
}

/** Read a bonding-curve VirtualPool's live state. */
export async function getPoolState(client: SuiClient, poolId: string): Promise<PoolState> {
...
export async function getAmmState(client: SuiClient, ammId: string): Promise<PoolState> {
...
export async function getLaunchConfig(client: SuiClient, configId: string): Promise<LaunchConfig> {
  const f = await fields(client, configId);
  const quoteName = String((f.quote_type as { fields: { name: string } }).fields.name);
  const quoteType = quoteName.startsWith('0x') ? quoteName : `0x${quoteName}`;
  const start = Number(BigInt(String(f.sqrt_start_price))) / TWO64;
  const pts = (f.curve as { fields: { sqrt_price: string; liquidity: string } }[]).map((p) => ({
    sqrt: Number(BigInt(p.fields.sqrt_price)) / TWO64,
    liq: Number(BigInt(p.fields.liquidity)),
  }));
  const segments: CurveSeg[] = pts.map((p, i) => ({
