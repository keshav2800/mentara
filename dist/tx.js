...
async function mergeAndSplit(tx, client, sender, coinType, amount) {
    const coins = await client.getCoins({ owner: sender, coinType });
    if (coins.data.length === 0)
        throw new Error(`no ${coinType} coins for ${sender}`);
...
export async function buildBuyTx(args) {
    ...
    if (args.venue === 'curve') {
        if (!args.configId)
            throw new Error('configId required for curve buy');
...
export function buildClaimPartnerFeeTx(args) {
    ...
    if (args.poolId) {
        if (!args.configId)
            throw new Error('configId required to claim partner fee from the curve');
        tx.moveCall({
...
export function buildCreateConfigTx(addr, p) {
    const m = mods(addr);
